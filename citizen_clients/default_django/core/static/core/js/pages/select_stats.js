// Page script: Select Poll Office (Stats)
// - Uses Select2 with remote search to /api/polloffices/?search=...
// - Navigates to poll-office-stats with selected id

(function () {
  function log(...args) { console.log('[SelectStats]', ...args); }
  ...
  $(function () {
    ...
    function saveSelectedToStorage(office) {
      try {
        ...
        console.warn('[SelectStats] Failed to save to localStorage', e);
      }
    }
    ...
    $button.on('click', () => {
      if (!selected) return;
      const statsBase = document.body.getAttribute('data-url-poll-stats') || '/poll-office-stats/';
      const href = `${statsBase}?id=${encodeURIComponent(selected.id)}`;
