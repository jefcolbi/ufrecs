// Page script: Poll Office Results
// - Reads ?id=
// - Fetches candidate parties (paged) and office results periodically

(function () {
  function log(...args) { console.log('[PollOfficeResults]', ...args); }
  ...
  $(async function () {
    ...
    // Resolve office name for title: prefer localStorage, fallback to API
    (function setTitleFromStorageOrApi() {
      ...
      if (name) {
        ...
        $.ajax({ url: '/api/polloffices/', method: 'GET', dataType: 'json', data: { search: id } })
          .done(resp => { const list = (resp && resp.results) || []; const m = list.find(o => o.id === id); $title.text(m ? `Results • ${m.name || m.identifier || ('Office ' + id)}` : `Results • Office ${id}`); })
          .fail(() => { $title.text(`Results • Office ${id}`); });
      }
    })();
    ...
    try {
      ...
      parties.forEach(p => { const pid = p.identifier || p.id; if (pid != null) partyById[pid] = p; });
      log('Loaded candidate parties', parties.length);
    } catch (err) { log('Failed loading candidate parties', err); }

    async function load() {
      try {
        const data = await $.ajax({ url: '/api/pollofficeresults/', method: 'GET', dataType: 'json', data: { poll_office: id } });
